...
def create_my_label_colormap():
    """Creates a label colormap used in PASCAL VOC segmentation benchmark.
    Returns:
      A colormap for visualizing segmentation results.
    """
    colormap = np.zeros((7, 3), dtype=int)
...
def create_potsdam_colormap():
    ...
    colormap[0,:] = [68, 1, 84]
    colormap[1,:] = [33, 145, 140]
    colormap[2,:] = [253, 231, 37]
...
class Potsdam(Dataset):
    def __init__(self, root, image_set, transform, target_transform, coarse_labels):
        ...
        split_files = {
            ...
            "all_train": ["labelled_train.txt", "unlabelled_train.txt"],
    ...
    def __getitem__(self, index):
        ...
        mask = (label > 0).to(torch.float32)
        # return img, label, mask
        return img, label, str(index).zfill(4)
...
class Coco(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.coarse_labels:
            ...
            return img, coarser_labels, image_path.split('/')[-1].split('.')[0] # for printing the results
        else:
            if self.exclude_things:
                ...
                return img, coarse_label, image_path.split('/')[-1].split('.')[0] # for printing the results
...
class CityscapesSeg(Dataset):
    ...
    def __getitem__(self, index):
        if self.transform is not None:
            ...
            return image, target.squeeze(0), str(index).zfill(4) # for printing the results
        else:
